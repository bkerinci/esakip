...
export default function Perencanaan({ dokumen, opds }) {
    return (
        <PublicLayout>
            <Head title="Perencanaan Kinerja Publik" />

            {/* Header Banner */}
            <div className="bg-slate-800 py-12 border-b-4 border-blue-500">
                <div className="container mx-auto px-4 text-center">
                    <h1 className="text-3xl md:text-4xl font-bold text-white mb-4">Dokumen Perencanaan Kinerja</h1>
                    <p className="text-slate-300 max-w-2xl mx-auto">
                        Akses dokumen publik perencanaan strategis, target, dan indikator kinerja utama Pemerintah Kota Sungai Penuh. Transparansi untuk akuntabilitas.
                    </p>
                </div>
            </div>
                                ...
                                <div key={opd.id} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                                    ...
                                    {opd.documents && opd.documents.length > 0 ? (
                                                    ...
                                                    {opd.documents.map(doc => (
                                                        <tr key={doc.id} className="hover:bg-slate-50">
                                                            <td className="p-3 font-medium text-slate-700">
                                                                {doc.jenis_dokumen === 'renstra' && 'Renstra'}
                                                                {doc.jenis_dokumen === 'perjanjian_kinerja' && 'Perjanjian Kinerja'}
                                                                {doc.jenis_dokumen === 'iku' && 'IKU'}
                                                            </td>
                                        ...
                                        <div className="text-center py-6 bg-slate-50 rounded border border-dashed border-gray-200">
                                            <p className="text-gray-400 text-sm italic">Belum ada dokumen Perencanaan, PK, atau IKU yang diunggah.</p>
                                        </div>
                                    )}
                                </div>
                            ))}
